...
uint8_t address 									 = 0;
uint8_t rx_byte_uart4							 = 0;
uint8_t rx_byte_uart5							 = 0;
...
KalmanFilter kf_soc;  																// Kalman Filter cho SOC
KalmanFilter kf_soh;  																// Kalman Filter cho SOH
...
void BMS_Init_Address(void) {
    ...
    if (huart5.Init.BaudRate != target_baud) {
        ...
        HAL_UART_Receive_IT(&huart5, &rx_byte_uart5, 1);
    }
		if (__HAL_RCC_GET_FLAG(RCC_FLAG_IWDGRST) != RESET && m == 0) {
        is_auto_coding = 0;
        ...
        uint16_t saved_addr = BKP->DR1; 
        
        if (saved_addr >= 0x03 && saved_addr <= 0x12) {
            pylon_protocol_addr = (uint8_t)saved_addr; 
        } else {
            pylon_protocol_addr = 0x03; 
        }
    }
		else if (m == 0) {
        is_auto_coding = 1;
...
uint8_t BMS_Flash_Init(void) 
{
    ...
    for (uint32_t offset = 0; offset < FLASH_EMERGENCY_SIZE; offset += FLASH_SECTOR_SIZE) 
		{
        uint32_t erase_addr = FLASH_EMERGENCY_START + offset;
        if (w25qxx_sector_erase_4k(&flash_handle, erase_addr) != 0) {
...
uint8_t BMS_Load_Data_From_Flash(void)
{
    ...
    for (uint8_t i = 0; i < WEAR_LEVEL_SECTORS; i++) {
        ...
        if (w25qxx_read(&flash_handle, addr, (uint8_t*)&temp_data, sizeof(bms_flash_data_t)) != 0) {
            continue;
        }
    ...
    for (uint16_t i = 0; i < FLASH_EMERGENCY_ENTRIES; i++) {
		...
		if (found_valid) {
						...
						return restore_data_from_flash(&best_data);
				}
				current_sequence = 0;
				current_wear_slot = 0;
...
void BMS_Smart_Parallel_Control(void) {
    ...
    if (OV_Fault || UV_Fault || OCC_Fault || OCD_Fault || OCD_Fault1 || SCD_Fault ||
        ...
        if (ov_recovery_locked) cmd_chg[0] = 0;
        if (uv_recovery_locked) cmd_dsg[0] = 0;
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart4, &rx_byte_uart4, 1);
  HAL_UART_Receive_IT(&huart5, &rx_byte_uart5, 1);
...
//	}
  ...
  {       
			...
			if (!BMS_IsResetting() && !system_is_shutting_down)
      {
          ...
          {
              if (abs((int16_t)(curr_soc_x100 - last_saved_soc_x100)) >= FLASH_CHANGE_THRESHOLD) need_save = 1;
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    ...
    {		
        ...
        if (huart->Instance == UART4) {
            HAL_UART_Receive_IT(&huart4, (uint8_t *)&rx_byte_uart4, 1);
        } 
        else if (huart->Instance == UART5) {
            HAL_UART_Receive_IT(&huart5, (uint8_t *)&rx_byte_uart5, 1);
        }
...
void HAL_PWR_PVD_Callback(void) {
    if (__HAL_PWR_GET_FLAG(PWR_FLAG_PVDO)) {
        if (data_saved_by_button || system_is_shutting_down) {
            HAL_GPIO_WritePin(GPIOC, POWER_HOLD_Pin, GPIO_PIN_RESET);
        }
        if (flash_init_success)
        {
