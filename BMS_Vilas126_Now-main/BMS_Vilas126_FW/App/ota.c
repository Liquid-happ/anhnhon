...
static uint16_t      vrui_GWIF_LengthMessage;
...
static const opcode_entry_t opcode_table[] = {
    { OP_START_OTA,  "OP_START_OTA",  handle_start_ota },
    { OP_GET_INFOR,  "OP_GET_INFOR",  handle_get_infor },
    { OP_PING_STM32, "OP_PING_STM32", handle_ping_stm32 }
};

#define OPCODE_COUNT (sizeof(opcode_table)/sizeof(opcode_table[0]))

static void opcode_dispatch(uint16_t opcode, uint8_t *data, uint16_t len)
{
...
static uint8_t is_valid_opcode(uint16_t opcode)
{
    for (int i = 0; i < OPCODE_COUNT; i++)
    {
        if (opcode_table[i].code == opcode)
...
static unsigned char Checksum_tts(TS_GWIF_IncomingData *data_1, unsigned int length)
{
...
void ota_init(void)
{
    ...
    ota_send_data_esp((uint8_t *)&ota_info.app_a_version, OP_GET_INFOR, 4);
}
...
void ota_check_data(void)
{
    ...
    if (vrts_ringbuffer_Data.count >= 1)
    {
        if (vrb_GWIF_UpdateLate == false)
        {
            if (vrb_GWIF_RestartMessage == true)
            {
                if (vrts_ringbuffer_Data.count >= 6)
                {
                    ...
                    vrui_GWIF_LengthMessage = (vrts_GWIF_IncomeMessage->Length[0] << 8) | (vrts_GWIF_IncomeMessage->Length[1]);
            ...
            else
            {
                ...
                vrui_GWIF_LengthMessage = (vrts_GWIF_IncomeMessage->Length[1]) | (vrts_GWIF_IncomeMessage->Length[0] << 8);
                vrui_GWIF_header = (vrts_GWIF_IncomeMessage->Header[0] << 8) | (vrts_GWIF_IncomeMessage->Header[1]);
            ...
            if (messageUpdate == true)
            {
                ...
                if ((vrts_GWIF_IncomeMessage->Start[0] == 0x55) && (vrts_GWIF_IncomeMessage->Start[1] == 0xaa))
                {
                    ...
                    {
                        if (vrts_ringbuffer_Data.count >= vrui_GWIF_LengthMessage + 1)
                        {
                            for (Count = 0; Count < vrui_GWIF_LengthMessage + 1; Count++)
                            {
        ...
        {
            if (vrts_ringbuffer_Data.count >= (vrui_GWIF_LengthMessage + MESSAGE_OFFSET))
            {
                for (vrui_Count = 0; vrui_Count < (vrui_GWIF_LengthMessage + MESSAGE_OFFSET); vrui_Count++)
                {
...
void ota_process(void)
{
    if (vrb_GWIF_CheckNow)
    {
        vrb_GWIF_CheckNow = false;
#if RD_DEBUG
        sprintf((char*)temp_debug, "Header: 0x%04X, Length: %d\r\n", vrui_GWIF_header, vrui_GWIF_LengthMessage);
        HAL_UART_Transmit(&uart_debug_id, temp_debug, strlen((char*)temp_debug), 100);
        sprintf((char*)temp_debug, "CRC Received: 0x%02X, CRC Calculated: 0x%02X\r\n", vrts_GWIF_IncomeMessage->Message[vrui_GWIF_LengthMessage], Checksum_tts(vrts_GWIF_IncomeMessage, vrui_GWIF_LengthMessage));
        HAL_UART_Transmit(&uart_debug_id, temp_debug, strlen((char*)temp_debug), 100);
#endif
        if (vrts_GWIF_IncomeMessage->Message[vrui_GWIF_LengthMessage] == Checksum_tts(vrts_GWIF_IncomeMessage, vrui_GWIF_LengthMessage))
        {
            opcode_dispatch(vrui_GWIF_header, vrts_GWIF_IncomeMessage->Message, vrui_GWIF_LengthMessage);
        }
    }
}
...
void handle_start_ota(uint16_t opcode, uint8_t *data, uint16_t len)
{
    ...
    HAL_UART_Transmit(&uart_debug_id, temp_debug, strlen((char*)temp_debug), 100);
    sprintf((char*)temp_debug, "START_OTA received: version=%u, size=%u\r\n", (unsigned int)Ota_data.version, (unsigned int)Ota_data.size);
    HAL_UART_Transmit(&uart_debug_id, temp_debug, strlen((char*)temp_debug), 100);
#endif
...
#endif
    ota_send_data_esp(NULL, OP_PING_STM32, 0);
}
...
void ota_run_check_uart(void)
{
    ...
    {
        // ota_run_received_ota (defined in Proto/xmodem.c)
        ota_run_received_ota();
    }
    else
    {
