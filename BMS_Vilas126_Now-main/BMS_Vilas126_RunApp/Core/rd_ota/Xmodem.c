...
extern HAL_StatusTypeDef CommandSubcommands(uint16_t command);
#define ALL_FETS_OFF 0x0022
...
void rd_xmodem_send_data(uint8_t *data, uint16_t length)
{
    HAL_UART_Transmit(&huart1, data, length, 100);
}


uint8_t Check_sum_recevice(uint8_t *data){
    ...
    switch(xmodem_state){    
        case RD_get_infor:
            if(Ota_data.ok_start_ota){
                ...
                rd_send_data_esp(NULL, OP_START_OTA, 0);
                #if RD_DEBUG
            ...
            if(vrts_ringbuffer_Data.count >=5){
                ...
                if(memcmp(infor, RD_INFOR, 5) == 0){
                    rd_xmodem_send_data((uint8_t *)RIIM_type, strlen(RIIM_type));
        ...
        case RD_get_infor_wait:
             if(vrts_ringbuffer_Data.count >=5){
                ...
                if(memcmp(imag, RD_IMAG1, 5) == 0){
                    rd_xmodem_send_data((uint8_t *)&X_NAK, 1);
            ...
            if(vrts_ringbuffer_Data.count >= FLASH_PAGE_SIZE + 4){
                ...
                if(data_flash_in->start == 0x01){
                    ...
                    if(check_block_number == data_flash_in->check_number_of_block){
                        if(Check_sum_recevice(data) == data_flash_in->check_sum_block){
                           // rd_flash_write(data_flash_in->data, 128);
                           // nhan du, dung data
                            // #if RD_DEBUG
                            // sprintf((char*)temp_debug, "Block %d coming\r\n", data_flash_in->number_of_block);
                            // HAL_UART_Transmit(&uart_debug_id, temp_debug, strlen((char*)temp_debug), 100);
                            // #endif
                            ...
                            Ota_data.TimeoutCounter = temp_tick;
                            rd_xmodem_send_data((uint8_t *)&X_ACK, 1);
                            #if RD_DEBUG
...
//                    xmodem_state = RD_end;
        ...
        case RD_end:
            if(vrts_ringbuffer_Data.count >=1){
                ...
                if(end_byte == X_EOF[0]){
                    ...
                    #endif
                    HAL_Delay(100);
                    // o day co the them code de reset ve bootloader
                    NVIC_SystemReset();
                }
            }
            break;
        case RD_end_wait:
            HAL_Delay(100);
            if(have_err_ota == 0){
//                rd_xmodem_send_data((uint8_t *)RIIM_code_ok, strlen(RIIM_code_ok));
                ota_info.app_b_version = Ota_data.version;
                ota_info.app_b_size    = Ota_data.size;
               // ota_info.app_b_crc     = rd_crc32_flash(APP_B_ADDR, Ota_data.size);
                CalculateSHA256(APP_B_ADDR, Ota_data.size, ota_info.app_b_crc);
                if(rd_crc32_equal_volatile(ota_info.app_b_crc, Ota_data.crc, 32)){
                    ...
                    NVIC_SystemReset();
										...
										rd_xmodem_send_data((uint8_t *)"ERROR_SHA256\r\n", 14);
                    #if RD_DEBUG
                ...
                rd_xmodem_send_data((uint8_t *)"ERROR_OTA\r\n", 11);
                #if RD_DEBUG
