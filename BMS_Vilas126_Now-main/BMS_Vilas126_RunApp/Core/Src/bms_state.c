...
void Buzzer_Task_1ms(void)
{
    ...
    switch (current_buzzer_mode) {
        ...
        case BUZZER_MODE_ALARM:      off_time = 2750; break;
        default: return;
    }
    if (buzzer_is_on) 
    {
...
void Update_Alarms(void)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        if (CellVoltage[i] >= sys_alarm_cell_ov_mv)
        {
            bms_alarms.OV_Alarm = 1;
        }
				if (CellVoltage[i] <= sys_alarm_cell_uv_mv)
        {
            bms_alarms.UV_Alarm = 1;
        }
    }
		
    if (Stack_Voltage >= sys_alarm_pack_ov_mv)
    {
        bms_alarms.Stack_OV_Alarm = 1;
    }
		
    if (Stack_Voltage <= sys_alarm_pack_uv_mv)
    {
        bms_alarms.Stack_UV_Alarm = 1;
    }

    if (Pack_Current >= sys_alarm_occ_10ma && bms_state == BMS_STATE_CHARGING)  
    {
        bms_alarms.OCC_Alarm = 1;
    }
		
    if (Pack_Current <= sys_alarm_ocd_10ma && bms_state == BMS_STATE_DISCHARGING) 
    {
    ...
    if (cell_temp >= sys_alarm_otc_c && Pack_Current > 50)
    {
        bms_alarms.OTC_Alarm = 1;
    }
		
    if (cell_temp >= sys_alarm_otd_c && Pack_Current <= -50)
    {
        bms_alarms.OTD_Alarm = 1;
    }
		
    if (min_temp <= sys_alarm_utc_c && Pack_Current > 50)
    {
        bms_alarms.UTC_Alarm = 1;
    }
		
    if (min_temp <= sys_alarm_utd_c && Pack_Current <= -50)
    {
        bms_alarms.UTD_Alarm = 1;
    }
		
    if (env_temp_local >= sys_alarm_eot_c)
    {
        bms_alarms.EOT_Alarm = 1;
    }
		
    if (env_temp_local <= sys_alarm_eut_c)
    {
        bms_alarms.EUT_Alarm = 1;
    }

    if (fet_temp >= sys_alarm_otf_c)
    {
        bms_alarms.OTF_Alarm = 1;
    }

    if (SOC < system_low_soc_alarm && bms_state != BMS_STATE_CHARGING)
    {
    ...
    if (max_cell - min_cell > sys_alarm_cell_diff_mv)
    {
