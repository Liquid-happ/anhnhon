...
float system_low_soc_alarm 				 = 5.0f;
uint16_t sys_alarm_cell_ov_mv   	 = 3600;
uint16_t sys_alarm_cell_uv_mv  		 = 2800;    
uint16_t sys_alarm_pack_ov_mv   	 = 57600;  
uint16_t sys_alarm_pack_uv_mv   	 = 44800;
int16_t  sys_alarm_occ_10ma     	 = 12000;  
int16_t  sys_alarm_ocd_10ma     	 = -12000;
int8_t   sys_alarm_otc_c        	 = 60;     
int8_t   sys_alarm_otd_c        	 = 65;     
int8_t   sys_alarm_utc_c        	 = 5;       
int8_t   sys_alarm_utd_c        	 = -15;    
int8_t   sys_alarm_eot_c        	 = 65;      
int8_t   sys_alarm_eut_c        	 = -15;     
int8_t   sys_alarm_otf_c        	 = 90;
uint16_t sys_alarm_cell_diff_mv 	 = 1000;
...
uint32_t system_total_remain_capacity_mAh 	= 0;
uint32_t system_total_nominal_capacity_mAh 	= 0;
...
int16_t local_max_temp_C           = 0;
int16_t local_min_temp_C           = 0;
uint8_t local_max_t_cell_id        = 0;
uint8_t local_min_t_cell_id        = 0;
...
float debug_voltage								 = 0;
float debug_r_lower								 = 0;
float debug_resistance						 = 0;
float debug_temp									 = 0;
...
DS1307_TIME time;
uint8_t slave_clamp_status[MAX_SLAVES] 						  = {0};
uint8_t slave_stop_status[MAX_SLAVES] 						  = {0};
...
void Update_SOC_SOH_FromBQ(void)
{
    ...
    else if (adv_soc_clamp_active && SOC > 20.0f) 
    {
        SOC -= 0.01f;
    ...
    if (!UV_Fault && !OV_Fault && !uv_recovery_locked && !ov_recovery_locked && !adv_soc_clamp_active && !adv_stop_discharge_active) {
        ...
        SOC = fmaxf(0.0f, fminf(100.0f, SOC));
				if (SOC < 21.0f && local_min_cell_v > 3200) {
            SOC = 21.0f;
            SOC_Capa = 21.0f;
            currentUsage = FullChargeCapacity_mAh * (1.0f - (SOC / 100.0f));
    ...
    for (uint8_t i = 0; i <= MAX_SLAVES; i++) {
        ...
        if (!pchg_active[i]) {
            if (diff_from_max > THRESH_HIGH) {
                ...
                if (i == 0) {
										uint8_t master_local_dsg = DSG && !adv_stop_discharge_active;
                    cmd_chg[0] = CHG; 
                    cmd_dsg[0] = master_local_dsg; 
    ...
    for (uint8_t i = 0; i < MAX_SLAVES; i++) {
        if (slave_online_status[i] > 0) {
            ...
            cmd.set_charge_i_limit    = (cmd.stt.charge_en) ? (system_charge_limit_A) : 0;
						if (slave_stop_status[i] || slave_clamp_status[i]) {
								cmd.set_discharge_i_limit = 0;
								cmd.stt.discharge_en = 0;
						} else {
								cmd.set_discharge_i_limit = system_discharge_limit_A;
								cmd.stt.discharge_en = 1;
						}
...
void BMS_Slave_FET_Control(void) {
    ...
    uint8_t local_dsg = !uv_recovery_locked && !adv_stop_discharge_active;
    ...
    if (desired_state != actual_state) {
        if (net_pchg == 0 && current_pchg_pin == GPIO_PIN_SET) {
            ...
            else if (final_dsg == 0) CommandSubcommands(DSG_PDSG_OFF); // Thực thi Van 1 chiều
            else CommandSubcommands(ALL_FETS_ON);
        }
    }
...
void BMS_Aggregate_System_Data(void) {
    ...
    local_max_temp_C = -273;
    local_min_temp_C = 1000;
    local_max_t_cell_id = 1;
    local_min_t_cell_id = 1;
    ...
    for (int t = 0; t < 8; t++) {
        ...
        if (m_temp > local_max_temp_C) { 
            local_max_temp_C = m_temp; 
            local_max_t_cell_id = t + 1; 
        }
        if (m_temp < local_min_temp_C) { 
            local_min_temp_C = m_temp; 
            local_min_t_cell_id = t + 1; 
        }
    }
		
		int16_t g_max_temp = local_max_temp_C;
    int16_t g_min_temp = local_min_temp_C;
    uint8_t g_max_t_pack_id = 1; 
    uint8_t g_max_t_cell_id = local_max_t_cell_id;
    uint8_t g_min_t_pack_id = 1; 
    uint8_t g_min_t_cell_id = local_min_t_cell_id;
    ...
    sys_alarm_uv  = bms_alarms.UV_Alarm || bms_alarms.Stack_UV_Alarm || adv_stop_discharge_active;
    ...
    for (int i = 0; i < MAX_SLAVES; i++) {
				if (slave_mgmt_data[i].discharge_current_limit == 0 && Pack_Current < -50) {
            slave_stop_status[i] = 1;
        } else {
            slave_stop_status[i] = 0;
        }
        if (slave_online_status[i] > 0 && slave_isolated[i] == 0) {
            count++;
    ...
    system_total_capacity_mAh = (uint32_t)total_full_mah; 
		system_total_remain_capacity_mAh = (uint32_t)total_remain_mah;
		system_total_nominal_capacity_mAh = (uint32_t)((float)count * NOMINAL_CAPACITY_mAh);
    if (count > 0) {
        ...
        system_avg_soh = (uint16_t)roundf(total_soh_cap_mah * 100.0f / (float)system_total_nominal_capacity_mAh);
    }
    ...
    uint32_t active_dis_limit = adv_soc_clamp_active ? 0 : system_discharge_limit_A;
    if (local_min_cell_v <= 2800) active_dis_limit = 0;
		
    for (int i = 0; i < MAX_SLAVES; i++) {
        if (slave_online_status[i] > 0 && slave_isolated[i] == 0) {
            if (!slave_stop_status[i] && !slave_clamp_status[i]) {
                active_dis_limit += slave_mgmt_data[i].discharge_current_limit;
            }
        }
    }
    system_discharge_limit_A_final = (uint16_t)active_dis_limit;
...
void BMS_Advanced_Discharge_Logic(void)
{
    ...
    if (now - last_exec_tick < 250) return;
    last_exec_tick = now;
		local_max_cell_v = 0;
    local_min_cell_v = 0xFFFF;
    for(int i = 0; i < 16; i++) {
    ...
    uint16_t delta_cell = local_max_cell_v - local_min_cell_v;
		if (adv_soc_clamp_active) 
    {
        if (SOC <= 20.0f) {
    ...
    uint8_t is_discharging = (Pack_Current <= -50); 
    ...
    if (local_min_cell_v <= 3200 && is_discharging && SOC > 21.0f) 
    {
